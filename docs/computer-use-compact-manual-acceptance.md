...
| CU-02 | Strict runtime path | Runtime uses `verboo-computer-use` with the temporary MCP config; no external Computer Use skill, provider, AppleScript, JXA, or System Events path is used | MCP config serialization tests and helper capability contracts | Capture sanitized process/config diagnostic excerpt | NOT RUN |
