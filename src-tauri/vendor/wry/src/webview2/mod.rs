...
/// Yield to COM while waiting for a WebView2 async operation to complete.
///
/// A waitable event lets COM re-enter the STA and deliver completion callbacks
/// on the calling thread. A nested Win32-only message pump cannot guarantee
/// that re-entrancy and can deadlock while creating another controller.
fn co_wait_for_handle(event: HANDLE) -> Result<()> {
  unsafe {
    let wait_result = CoWaitForMultipleHandles(
      (COWAIT_DISPATCH_CALLS.0 | COWAIT_DISPATCH_WINDOW_MESSAGES.0) as u32,
      INFINITE,
      &[event],
    );
    let close_result = CloseHandle(event);
    wait_result?;
    close_result?;
  }
  Ok(())
