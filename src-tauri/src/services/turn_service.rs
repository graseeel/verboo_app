...
pub(crate) fn build_prompt_internal(request: &AgentTurnRequest, is_resume: bool) -> String {
    ...
    // legacy skills may describe external desktop-control providers, so none
    // of them are injected into the visual executor prompt.
...
fn build_computer_use_instructions(access_mode: &crate::models::types::AccessMode) -> Vec<String> {
    let mut lines = vec![
        ...
        "Never use an external desktop-control provider, skill, script, or command. Never use AppleScript, osascript, JXA, System Events, CGEvent synthesis, or another UI automation fallback. Do not search for, install, or repair any alternative desktop-control tool.".to_string(),
...
mod tests {
    ...
    fn computer_use_instructions_require_observed_verification_and_untrusted_ui() {
        ...
        assert!(instructions.contains("Never use an external desktop-control"));
    ...
    fn computer_use_turn_omits_external_skills_and_disables_the_skill_tool() {
        ...
        request.skills = vec![crate::models::types::SkillSummary {
            id: "user:external-desktop-control".into(),
            name: "external-desktop-control".into(),
            description: "Use an external provider for desktop control".into(),
            path: "/Users/test/.verboo/skills/external-desktop-control/SKILL.md".into(),
        ...
        assert!(!prompt.contains("Use skill \"external-desktop-control\""));
        let external_skill_path = "/Users/test/.verboo/skills/external-desktop-control/SKILL.md";
        assert!(!prompt.contains(external_skill_path));
        assert!(args
            .windows(2)
