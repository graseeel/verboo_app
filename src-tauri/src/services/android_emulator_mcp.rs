...
const MCP_NAME: &str = "verboo-android-emulator";
const MANAGED_MARKER: &str = "VERBOO_ANDROID_EMULATOR_MANAGED";
const VERSION_MARKER: &str = "VERBOO_ANDROID_EMULATOR_VERSION";
...
pub struct AndroidEmulatorMcpService {
    app_version: String,
...
impl AndroidEmulatorMcpService {
    pub fn new(app_data_dir: PathBuf, app_version: String) -> Result<Self, String> {
        ...
        let bundled_helper = executable
            ...
            .ok_or("android_emulator_mcp_bundled_helper_missing")?;
        let config_root = std::env::var_os("VERBOO_CONFIG_DIR")
            .map(PathBuf::from)
            .or_else(|| dirs::home_dir().map(|home| home.join(".verboo")))
            .ok_or("android_emulator_mcp_config_directory_unavailable")?;
        Ok(Self {
            app_version,
            integration_root: app_data_dir.join("android-emulator-integration"),
            bundled_helper,
    ...
    pub fn ensure_registered(&self) -> Result<(), String> {
        ...
        match self.registration_state(&helper)? {
            RegistrationState::Conflict => return Err("android_emulator_mcp_conflict".into()),
    ...
    fn remove_managed_entry_from_config(&self) -> Result<(), String> {
        ...
        .map_err(|_| "android_emulator_mcp_config_invalid".to_string())?;
    ...
    fn install_helper(&self, destination: &Path) -> Result<(), String> {
        if !self.bundled_helper.is_file() {
            return Err("android_emulator_mcp_bundled_helper_missing".into());
        }
        let parent = destination
            .parent()
            .ok_or("android_emulator_mcp_helper_parent_missing")?;
    ...
    fn require_success(&self, output: CliOutput) -> Result<(), String> {
        if output.success {
            ...
            Err("android_emulator_mcp_cli_failed".into())
        } else {
            Err(format!(
                "android_emulator_mcp_cli_failed: {}",
                output.stderr.trim()
            ))
        }
    }
    ...
    fn write_managed_entry(&self, helper: &Path) -> Result<(), String> {
        ...
        let root = config
            .as_object_mut()
            .ok_or("android_emulator_mcp_config_invalid")?;
        let servers = root
            .entry("mcpServers")
        ...
        if let Some(existing) = servers.get(MCP_NAME) {
            ...
            if !managed || !owned {
                return Err("android_emulator_mcp_conflict".into());
            }
        }
...
fn atomic_write_config(path: &Path, bytes: &[u8]) -> Result<(), String> {
    let parent = path
        .parent()
        .ok_or("android_emulator_mcp_config_parent_missing")?;
    fs::create_dir_all(parent).map_err(|error| error.to_string())?;
    let temporary = parent.join(format!(
        ".{}.{}.tmp",
        path.file_name()
...
mod tests {
    ...
    fn fixture(config: Option<Value>) -> (TempDir, AndroidEmulatorMcpService, Arc<FakeRunner>) {
        let temp = TempDir::new().unwrap();
        let bundled = temp.path().join("bundle").join(helper_filename());
        fs::create_dir_all(bundled.parent().unwrap()).unwrap();
        fs::write(&bundled, b"emulator-helper").unwrap();
        ...
        let service = AndroidEmulatorMcpService::with_dependencies(
            "1.2.3",
            temp.path().join("data/android-emulator-integration"),
            bundled,
            config_path,
            runner.clone(),
        );
    ...
    #[test]
    fn android_emulator_mcp_missing_entry_is_installed_and_registered() {
        let (_temp, service, runner) = fixture(None);
        service.ensure_registered().unwrap();

        let helper = service.managed_helper_path();
        assert_eq!(fs::read(&helper).unwrap(), b"emulator-helper");
        #[cfg(unix)]
        assert_eq!(
    ...
    #[test]
    fn android_emulator_mcp_foreign_entry_is_left_untouched() {
        let (_temp, service, runner) = fixture(Some(json!({
            "mcpServers": {
                MCP_NAME: {
                    "command": "/foreign/verboo-android-emulator",
                    "args": ["mcp"],
                    "env": {}
                }
            }
    ...
    #[test]
    fn android_emulator_mcp_managed_old_version_is_replaced_idempotently() {
        let temp = TempDir::new().unwrap();
        let root = temp.path().join("data/android-emulator-integration");
    ...
    #[test]
    fn android_emulator_mcp_registers_atomically_when_model_discovery_is_offline() {
        let temp = TempDir::new().unwrap();
        let bundled = temp.path().join("bundle").join(helper_filename());
        fs::create_dir_all(bundled.parent().unwrap()).unwrap();
    ...
    #[test]
    fn disabled_reconciliation_removes_only_the_managed_emulator_entry() {
        let (_temp, service, runner) = fixture(None);
        ...
        assert_eq!(
            config["mcpServers"]["user-owned"]["command"],
            "/usr/bin/example"
        );
        assert!(runner.calls.lock().unwrap().is_empty());
    }
    ...
    #[test]
    fn disabled_reconciliation_preserves_a_foreign_emulator_entry() {
        let (_temp, service, runner) = fixture(Some(json!({
            "mcpServers": {
                MCP_NAME: {
                    "command": "/foreign/verboo-android-emulator",
        ...
        assert_eq!(
            config["mcpServers"][MCP_NAME]["command"],
            "/foreign/verboo-android-emulator"
        );
        assert!(runner.calls.lock().unwrap().is_empty());
    }
}
