...
/// Returns the localized title/body for a given kind + language. Mirrors
...
/// Decides whether a completion notification should fire. Mirrors Electron's
/// `showCompletionNotification` (src/main/index.ts:553).
///
/// - `never` → never fire
...
pub fn should_fire_completion(
    ...
    match kind {
        NotificationKind::Done | NotificationKind::DoneError => match settings.completion_notifications {
            ...
            CompletionNotificationMode::Background => !window_focused,
...
pub fn fire_notification(
    ...
    if !should_fire_completion(settings, kind, window_focused) {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn completion_background_suppresses_when_focused() {
    ...
    fn fire_notification_wraps_text() {
        let settings = settings_en(CompletionNotificationMode::Always);
        let result = fire_notification(&settings, NotificationKind::Done, true);
    ...
    fn fire_notification_returns_none_when_suppressed() {
        let settings = settings_en(CompletionNotificationMode::Never);
        assert!(fire_notification(&settings, NotificationKind::Done, true).is_none());
    }
